...
class Rule extends \Magento\Rule\Model\ResourceModel\AbstractResource
{
    ...
    {
        $this->_init('elevate_microsite_rule', 'rule_id');
    }
    ...
    protected function _afterDelete(AbstractModel $rule)
    {
        if ($rule->getId()) {
            ...
            $connection->delete(
                $this->getTable('elevate_microsite_rule_category'),
                ['rule_id=?' => $rule->getId()]
            );
            $connection->delete(
                $this->getTable('elevate_microsite_rule_customer_group'),
                ['rule_id=?' => $rule->getId()]
            );
            $connection->delete(
                $this->getTable('elevate_microsite_rule_store'),
                ['rule_id=?' => $rule->getId()]
            );
            $connection->delete(
                $this->getTable('elevate_microsite_rule_product'),
                ['rule_id=?' => $rule->getId()]
            );
            $connection->delete(
                $this->getTable('elevate_microsite_rule_customer'),
                ['rule_id=?' => $rule->getId()]
            );
        }
     ...
     * @since 1.5.3 sql group by replaced with fetchPairs
     */
    public function getCategoryIdsFromCollection(\Elevate\Microsite\Model\ResourceModel\Rule\Collection $collection)
    {
    ...
    {
        return $this->getTable('elevate_microsite_rule_product');
    }
    ...
    {
        return $this->getTable('elevate_microsite_rule_customer');
    }

...
class Data extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    public function getRulesCustomerBySession(){
        ...
        $ruleGroupTable = $resource->getTableName('amasty_groupcat_rule_customer_group');
        $sql = "Select * FROM " . $ruleGroupTable . " where customer_group_id = ". $groupId;
        $rule_id = $connection->fetchOne($sql);
        if (!$rule_id) return false;
        $ruleInfo = $objectManager->get('Amasty\Groupcat\Model\RuleRepository');
    ...
    function getRuleProductInAdmin($sku){
        ...
        $resource = $objectManager->get('Magento\Framework\App\ResourceConnection');
        $connection = $resource->getConnection();
        $tableName = $resource->getTableName('amasty_groupcat_rule'); 
    ...
    function checkDisplayButtonProductDetail($_product){
        ...
        $_amastyHelper = $objectManager->get('Amasty\Groupcat\Helper\Data');
        $result = false;

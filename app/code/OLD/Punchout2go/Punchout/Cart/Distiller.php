...
class Distiller
{
    ...
    protected $lineItem, $product, $stashItem;
    ...
    public function getProductCustomOptions ()
    {
        if (!isset($this->productCache['custom_options'])) {
            ...
            $this->productCache['custom_options'] = array();
    ...
    public function getProduct()
    {
        if ($this->product === null) {
            $this->product = $this->context->getObjectManager()->create(Product::class)->load($this->getLineItem()->getProductId());
        }
        ...
        if (is_array($map) && !empty($map)) {
            ...
            foreach ($map AS $mapping) {
                  $value = $this->getCustomCartSourceValue($quote, $mapping['source']);
                   $this->helper->debug("--------Distiller::getCustomCartSourceValue | returned with::===> " . $value );
                  //NOTE: Where should these values be written to? Seems like we're just kind of copying
                  //key values and assigning them to new keys?
    ...
    public function getLineItemOptionValue($path)
    {
        ...
        $options = $this->getLineItem()->getProduct()->getTypeInstance(true)->getOrderOptions($this->getLineItem()->getProduct());
        ...
        if ($option) {
            $this->helper->debug("Distiller::getLineItemOptionValue | returning with value: " . serialize($option) );
        ...
        if ($product->hasData($path)
            ...
            if ($this->getLineItem()->getHasChildren()) {
                ...
                foreach ($children AS $child) {
                    ...
                    if ($product = $this->context->getObjectManager()->create(Product::class)->load($child->getProductId())) {
                        if ($product->hasData($path)
                            && $product->getData($path) != "") {
                            $attribute = $product->getResource()->getAttribute($path);
                            break;
    ...
    public function getLineItemStockValue($path){
        $returnValue = null;
        $stockItem = $this->context->getObjectManager()->get('\Magento\CatalogInventory\Model\Stock\StockItemRepository');
        $productStock = $stockItem->get($this->getLineItem()->getProductId());
        if($productStock && $productStock->hasData($path)) $returnValue = $productStock->getData($path);

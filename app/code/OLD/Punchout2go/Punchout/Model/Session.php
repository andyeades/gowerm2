...
class Session
{
    ...
    public function startSession()
    {
        ...
        $this->context->getEventManager()->dispatch('punchout_session_starting', array('session' => $this));
        ...
        $this->context->getEventManager()->dispatch('punchout_session_ready', array('session' => $this));
     ...
     * @throws \Exception
     */
    public function setupSession($input = array())
    {
        $this->helper->debug('Runing PunchOut Setup', $input);
        ...
        if (!$this->isPunchoutSession()) {
            throw new \Exception('Your punchout session request is invalid.');
        }
    ...
    public function getSessionStartUrl()
    {
        if ($this->_session_start_url == null) {
            ...
            $this->context->getEventManager()->dispatch('punchout_get_session_url', array('session' => $this));
...
//                array('_query' => 'posid=' . urlencode($this->getPunchoutSessionId())));
        ...
        $this->helper->debug('url parts '. json_encode($url_parts));
        // base parts
        $path = '';
        if (isset($url_parts['scheme'])) $path .= $url_parts['scheme'] ."://";
        if (isset($url_parts['host'])) $path .= $url_parts['host'];
        if (isset($url_parts['port'])) $path .= ":". $url_parts['port'];
        ...
        if (isset($url_parts['fragment'])) $path .= $url_parts['fragment'];

        // final
        $url = $path;
        // $url .= "posid=". urlencode($this->getPunchoutSessionId());
        $this->helper->debug('eval session - return');
        return $url;
    }
    ...
    public function isLevel2 ()
    {
        ...
        if (!isset($data['selected_item'])) {
            ...
            if ($selectedItem) {
                $data['selected_item'] = array ('sku' => $selectedItem);
            } else {
    ...
    public function destroySession ()
    {
        ...
        if ($userSession) {
            ...
            $userSession->destroy(array());
        }
        $checkoutSession = $this->getCheckoutSession();
        if ($checkoutSession) {
            $checkoutSession->destroy(array());
        }
    }
    ...
    public function setPunchoutData($data)
    {
        if (is_string($data)) {
            try {
                ...
                throw new \Exception('Your punchout data is invalid.');
            }
        }

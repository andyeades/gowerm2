...
class Uniquecart implements ObserverInterface
{
    ...
    public function execute(EventObserver $observer)
    {
        if ($this->punchoutSession->isPunchoutSession()) {
            ...
            // $this->punchoutSession->helper->debug('is punchout session:'. $eventName ."/". $this->punchoutSession->isInSetup());
            if ($eventName === 'custom_quote_process' && $this->punchoutSession->isInSetup()) {
                ...
                if ($quoteId !== $punchoutCart) {
                    $this->punchoutSession->helper->debug('non-matching. prepare new cart from : ' . $quoteId . ' not ' . $this->punchoutSession->getPunchoutCart());
                    $this->clearCurrentCart();
                    $quote = $this->createNewQuote();
                    // set new
                    $this->checkoutSession->setQuoteId($quote->getId());
    ...
    public function createNewQuote()
    {
        ...
        if ($this->customerSession->isLoggedIn()) {
            $this->punchoutSession->helper->debug('setting customer : '. $this->customerSession->getCustomer()->getId());

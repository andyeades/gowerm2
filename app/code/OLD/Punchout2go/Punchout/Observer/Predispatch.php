...
class Predispatch implements ObserverInterface
{
    ...
    public function execute(EventObserver $observer)
    {
        ...
        if ($this->helper->getConfigFlag('punchout2go_punchout/site/punchout_only') && !$this->punchoutSession->isPunchoutSession()) {
            ...
            if (!$this->testPassthrough($current, $passthrough) && strrpos($request->getRequestUri(),
                    $restricted_url) === false
            ) {
                $this->helper->debug('Restricting current : ' . $current);
                $url = $this->helper->getUrl($restricted_url);

...
class Data extends AbstractHelper
{
    ...
    {
        $array = array();
        $array['display']['return_link_label'] = $this->getConfig('punchout2go_punchout/display/return_link_label');
        $array['display']['return_link_enabled'] = (boolean) $this->getConfig('punchout2go_punchout/display/return_link_enabled');

        $array['session']['load_posdelay'] = $this->getConfig('punchout2go_punchout/session/load_posdelay');

        $array['session']['js_clear_localdata'] = (boolean) $this->getConfig('punchout2go_punchout/session/js_clear_localdata');
        $array['session']['js_session_clean'] = (boolean) $this->getConfig('punchout2go_punchout/session/js_session_clean');
        $array['session']['js_reload_sections'] = $this->getConfig('punchout2go_punchout/session/js_reload_sections');

        $array['session']['use_js_redirection'] = (boolean) $this->getConfig('punchout2go_punchout/session/use_js_redirection');
        $array['session']['edit_redirect_message'] = $this->getEditRedirectMessage();
        $array['session']['l2_redirect_message'] = $this->getL2RedirectMessage();
        $array['session']['redirect_timeout'] = $this->getConfig('punchout2go_punchout/session/redirect_timeout');

        $array['system']['js_logging'] = (boolean) $this->getConfig('punchout2go_punchout/system/js_logging');
    ...
    public function injectCartToSession($punchout_session, $cart, $store_manager)
    {
        ...
        //exit;
        // right now, we just want to call this.
    }
    ...
    public function evaluateBuyerRequest ($buyerRequest, $itemObj)
    {
        if (is_string($buyerRequest)) {
            ...
            if (empty($requestData)
                ...
                $requestData = unserialize($buyerRequest);
            }
        } else {
    ...
    public function updateShippingAddress ($data,$quote)
    {
        ...
        if (isset($data['first_name']) && !empty($data['first_name'])) $address->setFirstname($data['first_name']);
        if (isset($data['last_name']) && !empty($data['last_name'])) $address->setLastname($data['last_name']);
        if (isset($data['company']) && !empty($data['company'])) $address->setCompany($data['company']);
        if (isset($data['street']) && !empty($data['street'])) $address->setStreet($data['street']);
        if (isset($data['city']) && !empty($data['city'])) $address->setCity($data['city']);
        if (isset($data['telephone']) && !empty($data['telephone'])) $address->setTelephone($data['telephone']);
        ...
        if (isset($data['postcode']) && !empty($data['postcode'])) $address->setPostcode($data['postcode']);
        ...
        if ($session_type === 'login') {
            ...
            if (!empty($customer)) {
                ...
                if (is_numeric($id)) {
                    ...
                    //$customer_session->setCustomerDataAsLoggedIn($customer);
            ...
            if ($default_user) {
                ...
                $customer = $this->findCustomerByEmail($default_user, $store_manager->getWebsite()->getId(),
                    $punchout_session);
                if (!empty($customer)) {
                    $id = $customer->getId();
                    if (!empty($id)) {
                ...
                throw new \Exception('Unable to login without a default user.');
            }
        } else {
            // anonymous, session.
    ...
    public function makeCustomer($data, $default_group, $punchout_session, $store_manager)
    {
        ...
        $this->debug("Contact info=====>" . print_r($contactData, true));
        ...
        try {
            ...
            $this->eventManager->dispatch('punchout_new_customer_before_save', array(
                'customer'         => $customer,
            ...
            if (!preg_match('/[^\s@]+@[^\s@]+\.[^\s@]+$/', $customer->getEmail())) {
                throw new \Exception('Unable to create a new user without a valid email :' . ($customer->getEmail()));
            }
            ...
            $this->eventManager->dispatch('punchout_new_customer_after_save', array(
                'customer'         => $customer,
                'punchout_session' => $punchout_session,
                'punchout_data'    => $data,
                'default_group'    => $default_group,
    ...
    public function loadCustomer($data, $punchout_session, $store_manager)
    {
        ...
        $this->eventManager->dispatch('punchout_load_customer', array(
            'punchout_session' => $punchout_session,
        ...
        if (!$punchout_session->hasCustomer()) {
            ...
            $customer = $this->findCustomerByEmail($contactData['email'], $store_manager->getWebsite()->getId(),
                $punchout_session);
            $punchout_session->setCustomer($customer);
        } else {
     ...
     * @return \Magento\Customer\Model\Customer
     */
    public function findCustomerByEmail($email, $website_id = null, $punchout_session)
    {
        $this->debug('search : ' . $email . ' (' . $website_id . ')');
        /** @var \Magento\Customer\Model\Customer $customer */
        $c1 = $punchout_session->context->getObjectManager()->create('\Magento\Customer\Model\Customer');
        /** @var  $collection */
    ...
    private function cleanHeaderString($inputString)
    {
        ...
        foreach ($arrayCopy as $urlString) {
            ...
            $tempString = filter_var($urlString, FILTER_SANITIZE_URL,

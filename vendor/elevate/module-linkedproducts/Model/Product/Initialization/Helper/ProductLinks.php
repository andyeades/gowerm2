...
namespace Elevate\LinkedProducts\Model\Product\Initialization\Helper;

use Elevate\LinkedProducts\Model\Catalog\Product\Link;
...
class ProductLinks
{
     ...
     */
    const TYPE_NAME = 'linkedproducts';
    /**
    ...
    {
        if(isset($links[self::TYPE_NAME]) && !$product->getLinkedproductsReadonly()) {

            $links = (isset($links[self::TYPE_NAME])) ? $links[self::TYPE_NAME] : $product->getLinkedproductsLinkData();
            if (!is_array($links)) {
                $links = [];
            }

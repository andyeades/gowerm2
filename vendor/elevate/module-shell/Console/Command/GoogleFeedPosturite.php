...
class GoogleFeedPosturite extends Command
{
...
protected $scopeConfig;                              
    ...
    {
        $this->setName('elevate:shell:googlefeedposturite')->setDescription('run google feed');
    }
...
$this->_storeManager->setCurrentStore(4);
   ...
   // $productCollection->addAttributeToFilter('exclude_from_google_feed', 0);
    ...
    $productCollection->addAttributeToSelect('unspsc');
    $productCollection->addAttributeToSelect('brand');
      $productCollection->addAttributeToSelect('google_product_category');
                                         
                    $productCollection->addAttributeToSelect('gm_set_identifier_false');                        
      $productCollection->addAttributeToSelect('google_feed');    
    ...
    $productCollection->addUrlRewrite();
                            ...
                            // Sample sku array
//$productCollection->addAttributeToFilter('sku', array('in' => array('9899658')));
  $productCollection->addAttributeToFilter('google_feed', '1');
  $productCollection->setOrder('type_id', 'DESC');
    ...
    foreach ($productcollection as $products) {
      ...
      if (!is_null($image_link)) {
                  ...
                  $identifier_flag = $products->getData('gm_set_identifier_false');      
            $gtin = $products->getData('unspsc');
  ...
  protected function _getIdentifierExists( $product = NULL) {
    ...
    $identifier_flag = $product->getData('gm_set_identifier_false');      
    
    if($identifier_flag == '0'){
       return "false";
    }      
          
    $gmpn = $product->getData('unspsc');
...
if($is_sale_price){
 $price = $specialPrice;
  ...
  protected function _getProductType( $product = NULL) {
    ...
    return $product->getAttributeText('google_product_type');
  ...
  return $data;
    ...
    foreach ($data as $key => $value) {
      ...
      $str_rep = str_replace($bad, $good, $value);
      
      if(is_string($str_rep)){

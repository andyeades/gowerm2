...
class General extends AbstractHelper
{
     ...
     * @var mixed
     */
    protected $delivery_options_checkout_selectfirstdateoptionavailable;

    /**
     * @var mixed
     */
    protected $delivery_options_checkout_selectfirstdateavailable;
    ...
    public function __construct(
        ...
        \Elevate\Framework\Helper\Data $evHelper,
        ...
        $this->delivery_options_checkout_selectfirstdateavailable = intval($this->scopeConfig->getValue('delivery_options/checkout/selectfirstdateavailable'));
        $this->delivery_options_checkout_selectfirstdateoptionavailable = intval($this->scopeConfig->getValue('delivery_options/checkout/selectfirstdateoptionavailable'));
    ...
    public function getFeeForMethod(
        ...
        $searchCriteria = $this->evHelper->buildSearchCriteria($filters, $sortorder);
    ...
    public function checkDeliveryMethodValid(
        $delivery_method_id,
        $postcode,
        $country,
        $date,
        ...
        $set_method_id = $delivery_method_id;
        ...
        */
        ...
        if (!empty($date_array)) {
            ...
            // Can I make this dependent on something so it will work in both areas being called from?
            // As on multishipping it needs to be just the one cart item/or items being sent to location X
            ...
            $cart_productids = [];
            $cart_qty_data = [];
            $cart_attribute_sets_counts[] = [];

            $cart_items_handling_times = [];
            $cart_items_date_next_available = [];
            ...
            $delivery_methods_forarea_full = [];
            $delivery_area_fallbackmethods = [];
            $delivery_methods_matched = [];
            ...
            $deliveryRulesAllProductsItems = [];
            $deliveryRulesProductsItems = [];
            ...
            if (!empty($deliveryRulesProductsCount) || (!empty($deliveryRulesAllProductsCount))) {
                ...
                $delivery_rules_matched_ids = [];

                // Global Rules

                $deliveryRulesCombined = [];
                ...
                $combinationDeliveryRulesSingles = [];

                // Store Removed Methods in Array
                $delivery_methods_removed = [];
                ...
                if (!empty($deliveryRulesCount)) {
                    ...
                    foreach ($deliveryRulesItems as $deliveryRule) {
                        ...
                        if (strcmp("Total Basket Qty", $deliveryRuleType) == 0) {
                            ...
                            if (strcmp($deliveryRuleOperator, ">=") == 0) {
                                ...
                                if (intval($cart_total_quantity) >= $deliveryRuleQuantity) {
                                    ...
                                    } elseif (strcmp($deliveryRuleFunction, 'Remove') == 0) {
                                        // Remove Method X
                                    ...
                                    } elseif (strcmp($deliveryRuleFunction, 'Remove All But') == 0) {
                        ...
                        } elseif (strcmp("Product Qty", $deliveryRuleType) == 0) {
                            // TODO: CODE THIS!
                            // Based on the Quantity of a particular Product]
                            // Does the Basket Satisfy the Rule? if not then:
                            continue;
                        } elseif (strcmp("Product Attribute Set Qty", $deliveryRuleType) == 0) {
                    ...
                    if (!empty($deliveryRulesCount)) {
                        ...
                        foreach ($deliveryRulesItems as $deliveryRule) {
                            ...
                            if (strcmp("Meta Rule - Do Nothing", $deliveryRuleFunction) == 0) {
                                ...
                                if (strcmp($deliveryRuleOperator, ">=") == 0) {
                                } elseif (strcmp($deliveryRuleOperator, "==") == 0) {
            ...
            if (count($delivery_methods_matched) > 0) {
                if (isset($delivery_methods_matched[$set_method_id])) {
                    ...
                    // Return Error
                    $output = ['error_message' => 'Please select a different day/delivery option and try again.'];
                }
            } else {
                ...
                if ($fallback_methods_count > 0) {
                    ...
                    if (isset($delivery_methods_matched[$set_method_id])) {
                        if (isset($delivery_methods_matched[$set_method_id]['delivery_fees'][$actual_date])) {
                            ...
                            if (isset($delivery_methods_matched[$set_method_id]['delivery_fees'][$actual_date])) {
                                ...
                                $output = ['error_message' => 'Error please try again'];
                            }
                        }
                    } else {
                    ...
                    $output = ['error_message' => 'Please select a delivery method and try again.'];
                }
            }
    ...
    public function getDeliveryInfo(
        ...
        $test = 0;
        $output = '';
        $delivery_methods_matched_count = count($delivery_methods_matched);

        // This is where they mainly differ
        if ($delivery_methods_matched_count > 0) {
            $output = array(
                'success' => true,
                'methods_available_count' => $delivery_methods_matched_count,
                'delivery_methods' => $delivery_methods_matched,
                'cart_items_data' => $cart_items_data,
                'cart_items_data_more' => $cart_items_data_more
            );
    ...
    public function getHolidayDates($date_string) {
        $filters = array(
            array(
                'field'          => 'end_date',
                'value'          => $date_string,
    ...
    public function getDeliveryOutput(
        ...
        $delivery_options_checkout_selectfirstdateoptionavailable = $this->delivery_options_checkout_selectfirstdateoptionavailable;

        $delivery_options_checkout_selectfirstdateavailable = $this->delivery_options_checkout_selectfirstdateavailable;
        ...
        $seperateholidaydates = $this->getHolidayDates($date_string);
        ...
        foreach ($dayslist as $day_key => $day) {
            ...
            foreach ($deliverymethod_sorted_freecounts as $key => $value) {
                ...
                if (strtotime($day) >= $deliverymethod_delivery_date_first) {
                    ...
                    $mobile_delivery_element .= '<div class="fauxradio"><i class="fa fa-check"></i></div>';
        ...
        $mobile_day_options_dates = $this->buildMobileCalendarDates($mobile_day_options);

        $mobile_day_options_2 = $mobile_day_options_dates['mobile_calendar_output'];

        $mobile_day_options_first_day = $mobile_day_options_dates['mobile_calendar_output_first_day'];

        $mobile_day_options_allbutfirst = $mobile_day_options_dates['mobile_calendar_output_allbutfirst'];
        ...
        $mobile_first_day_options = '';

        $something = array(
            'mobile_day_options_firstday'    => $mobile_day_options_first_day,
            'mobile_day_options_exceptfirstday' => $mobile_day_options_allbutfirst,
    ...
    public function getMultiShipAddressDeliveryOutput($postcode, $country, $address_id) {

        $delivery_info = $this->getDeliveryInfo($postcode, $country);


        if ($delivery_info['success'] == true) {

            $deliverymethods = $delivery_info['delivery_methods'];
            $cart_items_data = $delivery_info['cart_items_data'];
            $cart_items_data_more = $delivery_info['cart_items_data_more'];

            $output = $this->getDeliveryOutput($deliverymethods, $cart_items_data, $cart_items_data_more);
        } else {
            ...
            if ($delivery_info['methods_available_count'] == 0) {
                $output['error_message'] = "No methods available";
            }
        }
    ...
    public function buildMobileCalendar($mobile_day_options) {
        ...
        foreach ($mobile_day_options as $key => $value) {
            ...
            $date_class = '';
            if ($count === 0) {
                $mobile_calendar_output .= '<div class="mobile-dates-container delivery-slider">';
                // Make Flag? - in association with the output!
                // Would Make first date selected on page launch
                //$date_class .= ' ';
                //$date_class .= ' active';
                if ($this->delivery_options_checkout_selectfirstdateavailable) {
                    $date_class .= ' active';
                }
            }
            ...
            $mobile_calendar_output .= '<div class="mobile-date '.$date_class.'" data-date="' . $key . '" data-number="' . $date_number . '" data-month="' . $date_month . '"><div class="mobile-date-inner ' . $class . '" data-date="' . $key . '" data-number="' . $date_number . '" data-month="' . $date_month . '">';

            $mobile_calendar_output .= '<div class="date-selector fauxradio date-radio-selector-' . $key . '"><i class="fa fa-check"></i></div>';
    ...
    public function getDeliveryCheckoutDescriptionOutput($descriptions) {
        ...
        foreach ($descriptions as $key => $value) {
            $output .= '<div class="delivery-desc-checkout-' . $key . ' delivery-desc">';
            $output .= $value;
            $output .= '</div>';
        }
    ...
    public function buildMobileCalendarDates($mobile_day_options) {
        ...
        $mobile_calendar_output = '';
        $mobile_calendar_output_first_day = '';
        $mobile_calendar_output_allbutfirst = '';

        $count = 0;

        $delivery_options_checkout_selectfirstdateoptionavailable = intval($this->delivery_options_checkout_selectfirstdateoptionavailable);
        ...
        foreach ($mobile_day_options as $key => $value) {
            ...
            $date_month = date('F', strtotime($key));
            $date_day = date('D', strtotime($key));
            $day_output .= '<ul class="day-options-' . $key . '">';
            if ($count === 0) {
                if ($delivery_options_checkout_selectfirstdateoptionavailable === 1) {
                    $day_output_class = 'selected';
                }
            }
            foreach ($mobile_day_options[$key]['options'] as $option) {
                if (array_key_exists('no_options', $mobile_day_options[$key])) {
                    $day_output .= '<li class="date- no-selectables '.$day_output_class.'">';
                } else {
                    $day_output .= '<li class="date- '.$day_output_class.'">';
                }
                $day_output .= $option;
                $day_output .= "</li>";
            }
            ...
            if ($count == 0) {
                ...
                $mobile_calendar_output_first_day .= $day_output;

            }

            $mobile_calendar_output .= $day_output;

            if ($count >= 1) {
                $mobile_calendar_output_allbutfirst .= $day_output;
            }
        ...
        return array(
            'mobile_calendar_output' => $mobile_calendar_output,
            'mobile_calendar_output_first_day' => $mobile_calendar_output_first_day,
            'mobile_calendar_output_allbutfirst' => $mobile_calendar_output_allbutfirst
        );
    }
    ...
    public function checkDateSuitability($date, $deliverymethod_non_deliverydays, $seperateholidaydates) {
        ...
        while($date_suitable != 1) {
            if (!key_exists($date->format('N'), $deliverymethod_non_deliverydays)) {

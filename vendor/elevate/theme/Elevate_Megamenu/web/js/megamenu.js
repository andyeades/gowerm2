...
(function ($, window) {
    ...
    ELEVATE.Megamenu = (function () {
        ...
        var menu_type = false;
        var MqL = 992;
        var transitionClass = 'ease-in-out';
        ...
        function init() {
            ...
            jQuery('<div class="ev-overlay"></div>').insertAfter("#nav-bar-outer");
            $('.page-wrapper').addClass(transitionClass);
            attachEvents();
            ev_moveNavigation();
        ...
        function ev_shownav() {
            ...
            $('#page-main-outer').addClass('nav-is-visible');
            $('#e-pn').removeClass('is-hidden').addClass('nav-is-visible');
            $('.ev-overlay').addClass('is-visible');
        }

        function ev_closenavigation() {
            ev_closeNav(true);
        }
        ...
        function attachEvents() {
            ...
            isTouchDevice = hasTouchCapabilities ? 'maybe':'nope';

//attach a once called event handler to window

            jQuery(window).one('touchstart mousemove click',function(e){
            ...
            $('.col-level-top-link').children('a').on('click', function (e) {
            ...
            $('.cd-nav-trigger').on('click', function (event) {
                ...
                if ($('#e-pn').hasClass('nav-is-visible')) {
                    ev_closenavigation();
                } else {
                    //open the menu
                    ev_shownav();
            ...
            $('.ev-overlay').on('swiperight', function () {
                if ($('.e-pn').hasClass('nav-is-visible')) {
                    ev_closenavigation();
                }
            });
            ...
            $('.ev-overlay').on('click', function () {
                ...
                if ($('#hdr-search-wrap').hasClass('is-visible')) {
                    $('#hdr-search-wrap').removeClass('is-visible');
                    $('.ev-overlay').removeClass('is-visible searchmodifier');
                }

            });

            $('.cd-nav-trigger-close').on('click', function (e) {
                ev_closenavigation();
            });


            $('#e-pn .has-children').children('a').on('click', function (e) {
                if (dragging) {
                    return;
                }
                ...
                if (selected.next('ul').hasClass('is-hidden')) {
                    //desktop version only ? Mobile
                    selected.addClass('selected').next('ul').removeClass('is-hidden').addClass('nav-opened').end().parent('.has-children').parent('ul').addClass('moves-out');
                    selected.parent('.has-children').siblings('.has-children').children('ul').addClass('is-hidden').end().children('a').removeClass('selected');
                    $('.ev-overlay').addClass('is-visible');
                } else {
                    selected.removeClass('selected').next('ul').addClass('is-hidden').removeClass('nav-opened').end().parent('.has-children').parent('ul').removeClass('moves-out');
                    $('.ev-overlay').removeClass('is-visible');
                }

            });

            $('#e-pn .ev_nav_level_1.has-children').hoverIntent(function (e) {
            ...
            $('#e-pn .clickable').on('click', function (e) {
                if (dragging) {
                    return;
                }
            ...
            $('#e-pn .go-back').on('click', function (e) {
                if (dragging) {
                    return;
                }

                e.stopImmediatePropagation();
                $(this).parent('ul').addClass('is-hidden').removeClass('nav-opened').parent('.has-children').parent('ul').removeClass('moves-out');
                return false;
            });

            /* Search & Account Header Side Buttons */

            $('#hdr-account').on('click', function (event) {
                ev_opennav(event);
            ...
            $('#header-mobile-search').on('click', function (e) {
                ...
                if ($('#hdr-search-wrap').hasClass('is-visible')) {
                    ...
                    $('#hdr-search-wrap').addClass('is-visible');
                    $('.ev-overlay').addClass('is-visible searchmodifier');
                }


                $('#hdr-search-wrap input').focus().removeClass('field-highlight');

                setTimeout(function () {
                    $('#hdr-search-wrap input').addClass('field-highlight');
                }, 0);
            });
        ...
        function ev_closeNav(move) {
            ...
            $('#e-pn .has-children ul').addClass('is-hidden');
            $('#e-pn .has-children a').removeClass('selected');
            $('.moves-out').removeClass('moves-out');
            $('.nav-opened').removeClass('nav-opened');
            $('.cd-nav-trigger').removeClass('nav-is-visible');

            setTimeout(function () {
                $('#e-pn').addClass('is-hidden').removeClass('nav-is-visible').css('-webkit-overflow-scrolling', '');
                $('.ev-overlay').removeClass('is-visible');
                $('#page-main-outer').removeClass('nav-is-visible');
            }, 300);
        ...
        function ev_opennav(event) {
            ...
            if ($('#page-main-outer').hasClass('nav-is-visible')) {
                ev_closeNav(true);
                $('.ev-overlay').removeClass('is-visible');
            } else {
                ev_shownav()
                ...
                $('#e-pn').addClass('nav-is-visible').css('-webkit-overflow-scrolling', 'touch');
                $('#page-main-outer').addClass('nav-is-visible');
                $('.ev-overlay').addClass('is-visible');
            }
        }

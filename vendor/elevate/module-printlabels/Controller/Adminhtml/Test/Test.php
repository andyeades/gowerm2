...
class Test extends \Magento\Backend\App\Action
{
    ...
    /**
     * @var \Elevate\PrintLabels\Api\HolidaydatesRepositoryInterface
     */
     ...
     */
    protected $printlabels_data_helper;

    /**
     * @var \Elevate\PrintLabels\Helper\OrderData
     */
    protected $printlabels_order_data_helper;
    ...
    /**
     * @var \Elevate\PrintLabels\Model\Order\Pdf\Shipment
     */
    protected $salesModelOrderPdfShipment;
     ...
     * @param \Elevate\PrintLabels\Helper\Data                                    $printlabels_data_helper
     * @param \Elevate\PrintLabels\Helper\OrderData                               $printlabels_order_data_helper
     * @param \Psr\Log\LoggerInterface                                            $logger
     * @param \Elevate\PrintLabels\Api\HolidaydatesRepositoryInterface            $holidayDates
     * @param \Elevate\PrintLabels\Api\PrintlabelsApiRepositoryInterface          $printlabelsApi
     * @param \Magento\Sales\Model\ResourceModel\Order\Shipment\CollectionFactory $shipmentCollectionFactory
     * @param \Elevate\PrintLabels\Model\Order\Pdf\Shipment                       $salesModelOrderPdfShipment
    ...
    public function __construct(
        ...
        \Elevate\PrintLabels\Helper\Data $printlabels_data_helper,
        \Elevate\PrintLabels\Helper\OrderData $printlabels_order_data_helper,
        \Psr\Log\LoggerInterface $logger,
        \Elevate\PrintLabels\Api\PrintlabelsApiRepositoryInterface $printlabelsApi,
        \Elevate\PrintLabels\Api\HolidaydatesRepositoryInterface $holidayDates,
        \Magento\Sales\Model\ResourceModel\Order\Shipment\CollectionFactory $shipmentCollectionFactory,
        \Elevate\PrintLabels\Model\Order\Pdf\Shipment $salesModelOrderPdfShipment,
        ...
        $this->printlabels_data_helper = $printlabels_data_helper;
        $this->printlabels_order_data_helper = $printlabels_order_data_helper;
        $this->logger = $logger;
        $this->printlabelsApi = $printlabelsApi;
        $this->holidayDates = $holidayDates;
        $this->shipmentCollectionFactory = $shipmentCollectionFactory;
        $this->salesModelOrderPdfShipment = $salesModelOrderPdfShipment;
    ...
    public function execute()
    {
        ...
        //$this->createGiftMessagePdf('1000092408', $gift_message, '', '3');
        //exit;
        ...
        */
        ...
        if (!isset($params['datasend'])) {
            $response = [];
            //TODO - Redundant if not using exclude functionality (or Error Will not make any sense!)
        ...
        $search_criteria = $this->searchCriteriaBuilder->addFilter(
            'increment_id',
            $order_ids,
            'IN'
        )->create();

        $orders = $this->orderRepository->getList($search_criteria);
        ...
        foreach ($orders as $order) {
            ...
            // Register Shipment with Carrier
            $info = $this->registerShipmentWithCarrier($order, $order_grid_data_workable);

            // Create a Shipment

            // What about checking if one already exits and how we handle that?

            $shipment = $this->shipOrder($order, $increment_id, $info);

            // Print Stuff

            $print = $this->printItems($order, $info);
            ...
            $carrier_success = $info['success'];
            $shipment_success = $shipment['magento_shipment']['success'];

            $print_configuration = $print['configuration'];

            $print_configuration_filtered = array_filter(
                $print_configuration,
                function ($value) {
            ...
            if (!empty($carrier_success) && !empty($shipment_success)) {
                // All good
                ...
                foreach ($print_configuration_filtered as $key => $value) {
                    if (!empty(intval($value))) {
                        if (empty(intval($print['print_node_responses'][$key]['success']))) {
                            $output_message .= $key . ' - print success';
                            $success_count++;
                        } else {
                            $output_message .= $key . ' - print failure';
                            $failure_count++;
                        }
                    }
                }

                $new_info['main_message'] = 'Created Shipment, Created DPD Label via API - ';

                $new_info['main_message'] .= $output_message;

                if (empty($failure_count)) {
                    $new_info['success'] = 1;
                } else {
                    $new_info['success'] = 0;
                    $new_info['partial_success'] = 1;
                }

                $this->logger->info($increment_id . ' ' . $new_info['main_message']);
            } else {
                $order_message_output = 'Problem ';
                $order_message_output_array = [];

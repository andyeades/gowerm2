...
class Shipment extends \Magento\Sales\Model\Order\Pdf\Shipment
{
    ...
    protected function insertOrder(&$page, $obj, $putOrderId = true)
    {
        ...
        $deliveryDate = $order->getDeliveryDateSelected();
        $outputDeliveryDate = date('l jS F Y', strtotime($deliveryDate));
        $page->drawText('DeliveryDate: ' . $outputDeliveryDate, 315, $top, 'UTF-8');

        $postcode = $order->getShippingAddress()->getPostcode();

        $page->drawText($postcode, 315, $top -= 20, 'UTF-8');


        $this->_setFontRegular($page, 10);
        $page->drawText(__('Order Date: ') . $this->_localeDate->formatDate(
        ...
        // TODO: IF ANYONE ELSE EVER WANTS THIS THIS WILL NEED TO BE A TOGGLE
        ...
        foreach ($shipments as $shipment) {
            ...
            if ($order->getId()) {
                if ($order->getGiftMessageId() != 0) {
                    //If there's no gift message, this will return 0. Need to catch it to prevent an exception.
                    if ($giftWrap = $this->getGiftMessages($order->getId())) {
                        ...
                        foreach ($this->string->split($giftWrap["message"], 90, true, true) as $_value) {
                            $page->drawText(strip_tags(trim($_value)), 25, $yGift, 'UTF-8');
                            $yGift -= 15;
                        }

    ...
    public function getMaximalPrice()
    {
        if (!$this->maximalPrice) {
            ...
            if ($this->product->getPriceType() == Price::PRICE_TYPE_FIXED) {
                ...
                $min = $customOptionPrice->getCustomOptionRange(true);
                $max = $customOptionPrice->getCustomOptionRange(false);
                $price += $max;
                $fakePrice = $min != $max ? $fakePrice + $min : $price;
            }
            $this->fakeMinimalPrice = $this->calculator->getMaxAmount($fakePrice, $this->product);

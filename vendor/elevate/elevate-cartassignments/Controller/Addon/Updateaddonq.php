 ...
 */
class Updateaddonq extends \Magento\Checkout\Controller\Cart
{
    /**
    ...
    public function execute()
    {
        ...
        $assignment_id = $this->getRequest()->getParam('assignment_id');
        $qty = $this->getRequest()->getParam('qty'); //updated qty

        //find the current item, we should do this via id
        $assignment = $this->_cartAssignmentsModal->load($assignment_id);

      //  $assignment = $this->cart->getQuote()->getItemById($parent_id);

        $addon = $this->_cartAddonsModal->load($assignment->getData('addon_id'));

        $cap_qty_type = $addon->getData('cap_qty_type');


        //this needs to update
        $assignment_data = $assignment->getData();


        $parent_item = $this->cart->getQuote()->getItemById($assignment_data['parent_quote_item_id']);

        //sets the pillows quantity
        //  $child_item->setQty($new_qty);
        //  $child_item->save();
        ...
        $linked_quote_item_id = $assignment_data['linked_quote_item_id'];
        $parent_quote_item_id = $assignment_data['parent_quote_item_id'];


        /* Main Product Item Update - Start */
        //load by the quote item id - e.g bed or chair
        $item = $this->cart->getQuote()->getItemById($linked_quote_item_id);   //current item id i.e chair / desk / bed
        if (!$item) {
            //create better handler for this in future - its hidden on FE -maybe to add an item if trying to increment
            $response['error'] = 'add_product';

            echo json_encode($response);
           // $this->getResponse()->setBody(Mage::helper('core')->jsonEncode($response));
            return; //        Mage::throwException($this->__('Quote item is not found.'));
        }
        //cap the product to the parent qty


        if($cap_qty_type == 'product'){
        ...
        $difference_qty = $qty - $assignment_qty; //difference via ajax request fires on keyboard, so user can have added 2 extra, 2 minus for example if pressed button twice quickly


        $update_qty = $item_qty + $difference_qty;
        ...
        if ($update_qty < 1) {
            $this->cart->removeItem($linked_quote_item_id);
        } else {
            $item->setQty($update_qty)->save();
        }

        /* Main Product Item Update - End */
        //echo $qty;
        /* Addon Product Updates - Start */
        ...
        $response['parent_quote_item_id'] = $parent_quote_item_id;
        $response['linked_quote_item_id'] = $linked_quote_item_id;
...
echo json_encode($response);
   exit;
        ...
        $total_data['grand_total_inc_vat'] = $grand_total_incvat;

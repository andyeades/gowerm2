 ...
 */
class Mattress extends \Magento\Framework\App\Action\Action {
    /**
    ...
    public function getAddonBlock() {
        ...
        $item_product_id = $quoteItemObjectMain->getProductId();
        $item_product_sku = $quoteItemObjectMain->getSku();
        ...
        foreach ($collection as $data) {
            ...
            $cp = $parentProduct;
            if ($option = $quoteItemObjectMain->getOptionByCode('simple_product')) {
            ...
            if (!$quoteItemObject && (($data->getData('cartassignments_id') == 'product') || ($data->getData('addon_type') == 'multibox'))) {
            ...
            if ($data->getData('addon_type') == 'product') {
                ...
                $productOutput .= "<span $show_div>" . $this->addonMultiBoxHtml(
                        ...
                        $quoteItemObjectMain, $data, $quoteItemAssignmentsObject, $quoteItemObject, $linked_id
                    ) . "</span>";
...
">View More Offers</div>
 ...
 jQuery(\'#readmoreitems_\'+myclass).addClass(\'hideme\').hide();
        ...
        if ($price_type == 'ex_vat') {
            ...
            $output_price_add = $quoteItemObjectMain->getRowTotal();
        } else {
            $output_price_add = $quoteItemObjectMain->getRowTotalInclTax();
        }
        if ($quoteItemObjectMain->getDiscountAmount() > 0) {
            $output_price_add = $output_price_add - ($quoteItemObjectMain->getDiscountAmount());
        }

        $qty = 1;
        $original_item_qty = $quoteItemObjectMain->getQty();
    ...
    public function addonMultiBoxHtml(
        ...
        // }
        $addon_id = $data['addon_id'];
        $product_id = $quote_item_parent->getProductId();
        $prody = $product_id;

        // 1291983
        $item_id = $quote_item_parent->getId();
        $already_added = false;
        $title = '';
        $description = '';
        ...
        if (!empty($title)) {
            $productOutput .= '<div style="font-weight: bold;margin-bottom: 10px;font-size: 14px;">' . $title . '</div>';
        }
        ...
        $collection = $objectManager->create('Elevate\ProductAddons\Model\ResourceModel\LinkedProducts\Collection')->addFieldToFilter('product_id', $product_id);
        ...
        if (array_key_exists($item_id . "_" . $data['addon_id'], $this->assigned_addons)) {

            $count = 0;
            // $count = count($this->assigned_addons[$item_id . "_" . $data->getAddonId()]);
            foreach ($this->assigned_addons[$item_id . "_" . $data['addon_id']] as $location => $subcollection) {
   ...
   position:relative;";
                    ...
                    $cartsingl = Mage::getSingleton('checkout/cart');
                    ...
                    $product_price = $quoteProduct->getData('price');
                    ...
                    $line_discounts_enabled = Mage::getStoreConfig('elevate_assignments/general/line_discounts');
            ...
            <div style="margin-top:10px;">
                ...
                <div class="col-md-3 col-xs-3 col-3" style="padding:0px;">

                ';

                    if ($has_quant > 0) {
                        /*Increment Code*/
                        $productOutput2 .= '<div style="margin-bottom:10px;">
        ...
        <script>ELEVATE.Assignments.attachEventHandlers();</script>';
                ...
                ' . $product_name . '' . str_replace('Was From', 'Was', str_replace('Now From', 'Now', $price_block)) . '


<div class="prod-price-offer">
              Offer Price              <span class="price ep3 ep99999" id="product-price-1291988">
                       ' . Mage::helper('checkout')->formatPrice($final_price * 0.9) . '                 </span>
            </div>

    ';

                    $productOutput2 .= '</div>
                <div class="right-options-col col-md-1 col-xs-7 col-2">

 <input onclick="removeAddon(\'' . $addon['assignment_id'] . '\');" style="top: 24px!important;right: 0px!important; margin: 0; width: 21px; position: absolute; height: 21px;" type="checkbox" checked="checked">
...
' . Mage::helper('checkout')->formatPrice($final_price * 0.9 * $has_quant) . '</div>';
                    ...
                    if ($line_discounts_enabled == '1' && $quoteItemObject) {
                        ...
                        $discountBreakdown = $quoteItemObject->getDiscountBreakdown();
    ...
    \">
  <div>$rule_description - Save " . Mage::helper('checkout')->formatPrice($reduct) . "</div>
  </div>";
                        }
                        if ($discount_count > 0) {
                            $productOutput2 .= '  <div style="clear:both;"></div>   <div style="
   ...
   ' . $offer . '
      ...
      ';
        ...
        if (array_key_exists($product_sku, $this->has_sku)) {
            ...
            $clickEvent = 'addItemToCart(' . $product_id . ', ' . $quote_item_id_parent . ', \'' . $location . '\', \'' . $remove_item_id . '\', false, \'' . $addon_id . '\');';
        } else {
...
';
    ...
    public function getHtmlRow(
        ...
        $price_block = ''; //eades
        $objectManager = \Magento\Framework\App\ObjectManager::getInstance();
        $_associatedProduct = $objectManager->create('Magento\Catalog\Model\Product')->load($_associatedProduct);

        $childProducts = $_associatedProduct->getTypeInstance()->getUsedProducts($_associatedProduct);
        ...
        foreach ($childProducts as $child) {
            ...
            if ($bed_size == $child->getChooseSize()) {
                ...
                foreach ($categories as $categoryId) {
                    ...
                    $_category = $_objectManager->create('Magento\Catalog\Model\Category')->load($categoryId);
        ...
        $prodimg = $helperImport->init($_associatedProduct, 'product_page_image_small')->setImageFile($_associatedProduct->getSmallImage()) // image,small_image,thumbnail
                                ->resize(380)->getUrl();

        $html .= '<div class="buytogether_item2">
                <div class="bt_i_in">

                  <div id="bt_item_name2_' . $product_id . '" class="bt_item_name"> ' . $product_name . ' &nbsp;<span  data-size="modal-lg"  class="evlightbox" data-json="1" data-body="/mattress/product/buytogether/id/' . $product_id . '" style="text-decoration: underline;color: #f0386c;cursor:pointer;">Show Information</span>
                  </div>
                    ...
                    <div class="add_item_left">
                      <div class="add_item_inner">
                        ...
                        <div id="bt_img_2_' . $product_id . '" class="the-img">
                          ...
                          data-src="' . $prodimg . '"/></span>
                        </div>
                      </div>
                    </div>
                    <div class="add_item_right">
                      <div class="bt-price-normal">
                        <span class="bt_price_save_text">Save:</span> <span class="bt_price_perc">&pound;' . number_format((float)$save_price, 2, '.', '') . '</span>
                        <span class="bt_price_text">Was: </span>
                        <span class="bt_price">&pound;' . number_format((float)$rrp_price, 2, '.', '') . '</span>
                      </div>
                      <div class="bt-price-final">
                        <span class="bt_price_final_text">Offer Price: </span>
                        <span class="bt_price_save">&pound;' . number_format((float)$now_price, 2, '.', '') . '</span>
                      </div>
...
';
    ...
    public function execute() {
        ...
        if ($_product->getId() == '1292245' || $_product->getId() == '1292243' || $_product->getId() == '1293256') {
            $bed_size = 2637; //"Single (3' x 6'3\")";
        }
        // do stuff here

        $bed_id = $product_id;

        $product = $objectManager->create('Magento\ConfigurableProduct\Model\ResourceModel\Product\Type\Configurable')->getParentIdsByChild($product_id);
        if (isset($product[0])) {
        ...
        $collection = $objectManager->create('Elevate\ProductAddons\Model\ResourceModel\LinkedProducts\Collection')->addFieldToFilter('product_id', $_product->getId());

        $isBuyTogether = false;
        $isBuyTogether2 = false;

        $_buyTogetherProducts = [];
        $_buyTogetherProducts2 = [];
        $_buyTogetherProducts3 = [];
        $isBuyTogether3 = false;
        $complete_hide = false;
        $isInCategory = false;
        foreach ($collection as $row) {
            ...
            if ($row['linked_addon_type'] == 'bottom') {
                $_buyTogetherProducts2[] = $row['linked_product_id'];
                $isBuyTogether2 = true;
...
';
        ...
        if (count($_buyTogetherProducts) > 0 && $type == 'top') {
            ...
            if (!empty($_buyTogetherProducts)) {
                foreach ($_buyTogetherProducts as $_associatedProduct) {
                    ...
                    if ($_product->getId() == '1291272' || $_product->getId() == '1291276' || $_product->getId() == '1285260' || $_product->getId() == '1289205' || $_product->getId() == '1286259' || $_product->getId() == '1285386' || $_product->getId() == '1292649') {
    ...
    public function addonBoxHtml(
        ...
        $quote_item_id_parent = $quoteItemObjectMain->getId();
        ...
        if (array_key_exists($quote_item_id_parent . "_" . $data->getData('cartassignments_id'), $this->assigned_addons)) {
            ...
            $addon_loop = $this->assigned_addons[$quote_item_id_parent . "_" . $data->getData('cartassignments_id')];
   ...
   ><i class="fa fa-info-circle"></i></div>';
        ...
        $productOutput .= '<span class="evlightbox "  data-size="modal-lg" data-title="Quick View" data-footer-type="div" data-body-type="url"
             data-body="/assignments/addon/cartassignments/id/' . $product->getId() . '" style="cursor:pointer !important; ' . $link_style . '">';
        ...
        if ($line_discounts_enabled == '1') {
            if ($data['discount_percentage'] > 0) {
                ...
                $productOutput .= ' <div style="" class="fullprice">
   ...
   */
            if (array_key_exists($quote_item_id_parent . "_" . $data->getData('cartassignments_id'), $this->assigned_addons)) {
                ...
                $clickEvent = 'removeAddon(\'' . $remove_item_id . '\');';
            } else {
        ...
        $productOutput .= '</div><input id="postcode_' . $quote_item_id_parent . "_" . $data->getData('cartassignments_id') . '" onclick="' . $clickEvent . '" class="inposittion" type="checkbox" ' . $checked . '> ';
        ...
        if ($line_discounts_enabled == '1' && $quoteItemObject) {
            ...
            foreach ($rules as $rule) {
                ...
                $current_basket_amount = $this->_coreSession->getCABasketRuleIds();
                ...
                $offer .= "<div class=\"couponbox\">
  <div>$rule_description - Save " . ($reduct) . "</div>
  </div>";
            }
            if ($discount_count > 0) {
                $offer_text = 'offer is';
...
">The following ' . $offer_text . ' applied to this product</div>
   ' . $offer . '
        ...
        if ($already_added) {
            ...
            $productOutput .= '<div class="addon_price_float floattop" data-qty="' . $qty . '" id="rowpriceprod_' . $spinner_code . '">

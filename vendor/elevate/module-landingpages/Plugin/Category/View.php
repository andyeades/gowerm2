...
//
                ...
                else if($ev_landingpages_top_desc_type == 'onload_default_ajax_ajax' || $ev_landingpages_top_desc_type == 'onload_no_ajax_ajax'){
                    $categoryDescription = $ev_landingpages_top_desc_ajax;
                }
                else if($ev_landingpages_top_desc_type == 'onload_default_ajax_no'){
                    $categoryDescription = '';
                ...
                $categoryDescription = $filterProvider->getBlockFilter()
...
/* this needs putting into a helper same as need to do for bottom_description.phtml */  
   ...
   if($ev_landingpages_btm_desc_type == 'onload_default_ajax_default'){
    ...
    else if($ev_landingpages_btm_desc_type == 'onload_default_ajax_ajax' || $ev_landingpages_btm_desc_type == 'onload_no_ajax_ajax') {
        $bottom_description = $ev_landingpages_btm_desc_ajax;
    }
    else if($ev_landingpages_btm_desc_type == 'onload_default_ajax_no'){
        $bottom_description = '';

    }
                $bottom_description = $filterProvider->getBlockFilter()
                                    ->setStoreId($storeManager->getStore()->getId())
        ...
        if($is_landing_page){
         //get landing page description - need to check how this is populating on the non ajax version
              ...
              // print_r($landing_page_data);
                ...
                $breadcrumbs = $response->getLayout()->getBlock('breadcrumbs')->toHtml();

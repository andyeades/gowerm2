...
class Category extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb
{
    ...
    /**
     * @param \Mirasvit\Kb\Model\Category|\Magento\Framework\Model\AbstractModel $category
     * @return void
     */
    ...
    public function getArticlesNumber(\Magento\Framework\Model\AbstractModel $category, $customerGroupId)
    {
        ...
        LEFT JOIN {$resource->getTableName('mst_core_urlrewrite')} urls ON urls.entity_id = art.article_id
        WHERE path LIKE '{$category->getPath()}%' AND urls.type = 'ARTICLE' and urls.store_id IN 
        (" . implode(',', [0, $storeId]) . ") AND art.is_active=1 AND (
            EXISTS (
                SELECT *
                ...
                WHERE art.article_id = article_customer_group_table.acg_article_id
                    AND article_customer_group_table.acg_group_id IN ('all', {$customerGroupId})
            ) OR NOT EXISTS (
                SELECT *
                FROM {$resource->getTableName('mst_kb_article_customer_group')} AS `article_customer_group_table`
                WHERE art.article_id = article_customer_group_table.acg_article_id
     ...
     *
     * @param Category|\Magento\Framework\Model\AbstractModel $category
     *
     * @return void
     * @throws \Magento\Framework\Exception\LocalizedException
     */

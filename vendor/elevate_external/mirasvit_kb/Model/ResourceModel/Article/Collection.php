    ...
    \Magento\Framework\Option\ArrayInterface
{
    ...
    {
        $this->_init('Mirasvit\Kb\Model\Article', 'Mirasvit\Kb\Model\ResourceModel\Article');
    }
...
// AND (category_table.path LIKE '%/$rootId/%' OR category_table.category_id = $rootId)
     ...
     * @return $this
     */
    public function addStoreIdNoCatFilter($storeId)
    {
        $rootId = (int)$this->kbData->getRootCategory($storeId)->getId();
        $this->getSelect()
             ->where("EXISTS (SELECT * FROM `{$this->getTable('mst_kb_article_store')}`
                AS `article_store` WHERE main_table.article_id = _article_store.as_article_id
                    AND (article_store.as_store_id IN ($storeId, 0) OR article_store.as_store_id IS NULL)
                )");
    ...
    public function addStoreIdAltFilter($storeId, $category_id)
    {
        $rootId = (int)$this->kbData->getRootCategory($storeId)->getId();
        $this->getSelect()
             ->where("EXISTS (SELECT * FROM `{$this->getTable('mst_kb_article_category')}`
                ...
                LEFT JOIN `{$this->getTable('mst_kb_article_store')}` `article_store`
                    ON article_store.as_article_id = article_category_table.ac_article_id
                WHERE main_table.article_id = article_category_table.ac_article_id
                    AND (category_table.path LIKE '%/$category_id/%' OR category_table.category_id = $category_id)
                    AND (article_store.as_store_id IN ($storeId, 0) OR article_store.as_store_id IS NULL)
                )");

        return $this;
    }
    public function addStoreIdAlt2Filter($storeId)
    {
        $rootId = (int)$this->kbData->getRootCategory($storeId)->getId();
        $this->getSelect()
             ->where("EXISTS (SELECT * FROM `{$this->getTable('mst_kb_article_category')}`

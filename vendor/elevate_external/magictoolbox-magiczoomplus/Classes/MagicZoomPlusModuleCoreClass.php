...
class MagicZoomPlusModuleCoreClass
{
    ...
    public function getHeadersTemplate($jsPath = '', $cssPath = null)
    {
        ...
        $headers = [];
        $headers[] = '<!-- Magic Zoom Plus Magento 2 module version v1.6.0 [v1.6.88:v5.3.5] -->';
    ...
    public function getMainTemplate($params)
    {
        ...
        return "<a id=\"MagicZoomPlusImage{$id}\" {$dataImage2x} class=\"MagicZoom\" href=\"{$img}\"{$group}{$link}{$title}{$options}>$savepercent $savepercent2 $el_guarantee<img class=\"no-sirv-lazy-load\" itemprop=\"image\" src=\"{$thumb}\" {$thumb2x}  alt=\"{$alt}\"{$width}{$height} />$soldout<div id=\"swipe-icon\"></div></a>{$message}";

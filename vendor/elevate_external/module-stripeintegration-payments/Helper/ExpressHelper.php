...
class ExpressHelper
{
    ...
    public function getCartItems($quote)
    {
        ...
        if (abs($quote->getShippingAddress()->getDiscountAmount()) >= 0.01) {
            $discountAmount = $useStoreCurrency ? $quote->getShippingAddress()->getDiscountAmount()
                : $quote->getShippingAddress()->getBaseDiscountAmount();

            $displayItems[] = array(
                'label' => __('Discount'),
                'amount' => $this->stripeHelper->convertMagentoAmountToStripeAmount($discountAmount, $currency)
            );
        }
